...
/*   Updated: 2025/10/27 11:52:57 by pandemonium      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_ray_on_mini_map(t_game *game)
{
	...
	while (x < WIDTH)
	{
		...
		&& (int)ray_x < map->width && map->map[(int)ray_y][(int)ray_x] != '1')
		{
			my_mlx_pixel_put(game->mlx.img, ray_x * TILE_SIZE, ray_y * TILE_SIZE, 0xFFFF00);
