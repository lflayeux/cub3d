...
/*   Updated: 2025/10/27 12:26:42 by pandemonium      ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_step_and_side_dist(t_render_3d *render, t_player *player)
{
        ...
        {
            render->step_x = -1;
            render->side_dist_x = (player->pos_x - render->map_x) * render->delta_dist_x;
        }
        else
        {
            render->step_x = 1;
            render->side_dist_x = (render->map_x + 1.0 - player->pos_x) * render->delta_dist_x;
        }
        if (render->ray_dir_y < 0)
        {
            render->step_y = -1;
            render->side_dist_y = (player->pos_y - render->map_y) * render->delta_dist_y;
        }
        else
        {
            render->step_y = 1;
            render->side_dist_y = (render->map_y + 1.0 - player->pos_y) * render->delta_dist_y;
        }
}
...
{
    if (render->side == 0)
	render->perp_wall_dist = (render->map_x - player->pos_x + (1 - render->step_x) / 2) / render->ray_dir_x;
	else
	render->perp_wall_dist = (render->map_y - player->pos_y + (1 - render->step_y) / 2) / render->ray_dir_y;
